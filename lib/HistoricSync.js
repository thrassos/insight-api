...
var BAD_GEN_ERROR = 'Bad genesis block. Network mismatch between Insight and solarcoind? Insight is configured for:';
...
function HistoricSync(opts) {
  ...
  this.rpc = new RpcClient(config.solarcoind);
      ...
      async.doWhilst(
        ...
        function(err) {
          ...
          if ( self.blockChainHeight  === blockInfo.height ||
            ...
            oldtip = tip;
            if (!tip)
              throw new Error('Previous blockchain tip was not found on solarcoind. Please reset Insight DB. Tip was:'+tip)
...
HistoricSync.prototype.prepareFileSync = function(opts, next) {
  ...
  if ( opts.forceRPC || !config.solarcoind.dataDir ||
    self.height > self.blockChainHeight * PERCENTAGE_TO_START_FROM_RPC) return next();


  try {
  self.blockExtractor = new BlockExtractor(config.solarcoind.dataDir, config.network);
  } catch (e) {
...
HistoricSync.prototype.showSyncStartMessage = function() {
  ...
  info('Got ' + self.height +
    ' blocks in current DB, out of ' + self.blockChainHeight + ' block at solarcoind');

  if (self.blockExtractor) {
    info('solarcoind dataDir configured...importing blocks from .dat files');
