...
var BAD_GEN_ERROR = 'Bad genesis block. Network mismatch between Insight and bitmarkd? Insight is configured for:';
...
function HistoricSync(opts) {
  ...
  this.rpc = new RpcClient(config.bitmarkd);
      ...
      async.doWhilst(
        ...
        function(err) {
          ...
          if ( self.blockChainHeight  === blockInfo.height ||
            ...
            oldtip = tip;
            if (!tip)
              throw new Error('Previous blockchain tip was not found on bitmarkd. Please reset Insight DB. Tip was:'+tip)
...
HistoricSync.prototype.prepareFileSync = function(opts, next) {
  ...
  if ( opts.forceRPC || !config.bitmarkd.dataDir ||
    self.height > self.blockChainHeight * PERCENTAGE_TO_START_FROM_RPC) return next();


  try {
  self.blockExtractor = new BlockExtractor(config.bitmarkd.dataDir, config.network);
  } catch (e) {
...
HistoricSync.prototype.showSyncStartMessage = function() {
  ...
  info('Got ' + self.height +
    ' blocks in current DB, out of ' + self.blockChainHeight + ' block at bitmarkd');

  if (self.blockExtractor) {
    info('bitmarkd dataDir configured...importing blocks from .dat files');
